 ...
 */
public class CordovaInferEngine2 extends ASTVisitor implements IInferEngine {
	...
	public CordovaInferEngine2(InferOptions inferOptions)
	{
		this.inferOptions=inferOptions;
	}

	public CordovaInferEngine2()
	{
	...
	public boolean visit(IFunctionCall functionCall) {
		...
		if(sChars != null && (new String(sChars)).equals("define") 
			...
			InferredType type = compUnit.findInferredType(arg0.source());
			if(type == null){
	...
	public void endVisit(IFunctionCall messageSend) {
		char[] sChars = messageSend.getSelector();
		if(sChars != null && (new String(sChars)).equals("define")) {
			defineType = null;
		}
		...
		if( assignmentExpr instanceof IFunctionCall ){
			...
			if(fCall.getSelector() != null && new String(fCall.getSelector()).equals("require")){
				if( fCall.getArguments()[0] instanceof IStringLiteral){
					...
					InferredType type = compUnit.findInferredType(objName.source());
					if(type == null ){
				...
				if( receiverType != null ){
					...
					if( !(attr != null && attr.type != null) ){
						...
						if (isKnownName && attr.type.isAnonymous)
						{
							...
							{
								compUnit.inferredTypesHash.removeKey(attr.type.getName());
//								attr.type.name=fRef.getToken();
								compUnit.inferredTypesHash.put(attr.type.getName(), attr.type);
							}
						}
	...
	protected boolean handleFunctionExpressionAssignment(IAssignment assignment)
	{
		...
		if (type!=null)	// isConstructor
		{
			if (this.inferOptions.useInitMethod)
			{
				...
				type.addConstructorMethod(type.getName(), methodDeclaration, nameStart);
...
//			}
				...
				if (returnStatement.getExpression()!=null)
				{
					...
					if (currentContext.currentMethod.getInferredType()==VoidType) {
						...
						if(type != null && shouldSetToAny) {
							...
							if( this.currentContext.currentMethod.getInferredType() != null && this.currentContext.currentMethod.getInferredType().getName() != null) {
								currentMethodInferredType = new String(this.currentContext.currentMethod.getInferredType().getName());
							}
							
							boolean returnTypesEqual = type.equals(currentContext.currentMethod.getInferredType());
							boolean returnTypeNamesEqual = (new String(type.getName())).equals(currentMethodInferredType);
							boolean returnTypeIsWellKnown = WellKnownTypes.containsKey(type.getName());
	...
	public boolean visit(IFunctionDeclaration methodDeclaration) {
		pushContext();
		if(defineType != null ){
			this.currentContext.setCurrentType(defineType);
			this.currentContext.currentMethod = methodDeclaration;
			this.currentContext.currentType.updatePositions(methodDeclaration.sourceStart(),
...
//		}
	...
	private void copyAnonymousTypeToNamed(InferredType inClass,
		...
		compUnit.inferredTypesHash.removeKey(inClass.getName());
